...
(function () {
  ...
  Fx.CSS.implement({
    ...
    prepare: function(element, property, values) {
      ...
      if (property == 'transform') {
        ...
        parsed = values.map(function (value) {
          return {value: parser.parse(value), parser: parser};
        })
      }
  ...
  px = px.concat(coordinates.map(function (side) {
    return px[0] + side
  }));
  generics = generics.concat(coordinates.map(function (side) {
    return generics[0] + side
  }));
  deg = deg.concat(coordinates.map(function (side) {
    return deg[0] + side
  })).concat(coordinates.map(function (side) {
    return deg[1] + side
  }));
      ...
      serve: function(transform) {
        ...
        deg.each(function (t) {
          ...
          if (transform[t] != null) {
            ...
            if (transform[t] instanceof Array) style += t + '(' + transform[t].map(function (val) {
              return val + 'deg';
            }) + ') ';
        ...
        px.each(function (t) {
          if (transform[t] != null) style += t + '(' + Array.from(transform[t]).map(function (value) {
            return value + 'px';
          }) + ') ';
        });
        generics.each(function (t) {
          if (transform[t] != null) style += t + '(' + transform[t] + ') ';
        });
