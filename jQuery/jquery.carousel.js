/* jQuery Carousel 0.9.9
 @author Thomas Lanciaux
 @author Pierre Bertet
 @contributor Antoine Goutenoir
 This software is licensed under the CC-GNU LGPL <http://creativecommons.org/licenses/LGPL/2.1/>
 */

(function($) {

  $.fn.carousel = function(paramsExtended) {
    ...
    var params = $.extend({
      ...
      nextBtn: null, // DOM Element, if set will be used as next button and skip other Btn options except onClick callback
      prevBtn: null, // DOM Element, if set will be used as prev button and skip other Btn options except onClick callback
      nextBtnOnClick: null, // Function called before going next, return false to cancel slide
      prevBtnOnClick: null, // Function called before going prev, return false to cancel slide
      nextBtnHTML: '<input type="button" value="Next" />',
      prevBtnHTML: '<input type="button" value="Previous" />',
      btnsPosition: "inside",
    ...
    return this.each(function() {
      ...
      // Prev Button
      if (env.params.prevBtn != null) {
        env.$elts.prevBtn = $(env.params.prevBtn);
      } else {
        if ($.isFunction(env.params.prevBtnInsertFn)) {
          env.$elts.prevBtn = env.params.prevBtnInsertFn(env.$elts);
        } else {
          if (params.btnsPosition == "outside") {
            env.$elts.prevBtn = $(params.prevBtnHTML)[params.prevBtnInsert](env.$elts.carousel);
          } else {
            env.$elts.prevBtn = $(params.prevBtnHTML)[params.prevBtnInsert](env.$elts.wrap);
          }
        }
      }

      // Next Button
      if (env.params.nextBtn != null) {
        env.$elts.nextBtn = $(env.params.nextBtn);
      } else {
        if ($.isFunction(env.params.nextBtnInsertFn)) {
          env.$elts.nextBtn = env.params.nextBtnInsertFn(env.$elts);
        } else {
          if (params.btnsPosition == "outside") {
            env.$elts.nextBtn = $(params.nextBtnHTML)[params.nextBtnInsert](env.$elts.carousel);
          } else {
            env.$elts.nextBtn = $(params.nextBtnHTML)[params.nextBtnInsert](env.$elts.wrap);
          }
        }
      }
  ...
  function initButtonsEvents(env) {
    ...
    .bind("enable", function() {
      ...
      .bind("click", function() {
        ...
        // Call onClick callbacks
        var continueIfNotFalse = true;
        if ($.isFunction(env.params.nextBtnOnClick) && $this.is('.next')) {
          continueIfNotFalse = env.params.nextBtnOnClick($this);
        }
        if ($.isFunction(env.params.prevBtnOnClick) && $this.is('.previous')) {
          continueIfNotFalse = env.params.prevBtnOnClick($this);
        }

        if (continueIfNotFalse != false) {
          // Ajax init
